...
## Frontend mínimo funcional incluído nesta versão

Esta versão inclui um frontend React/Vite mínimo em `frontend/`, com:

- formulário sintético de identificação do ativo;
- sliders para os 28 indicadores MACA;
- cálculo local de PP, PFA, FI, FD, PFA* e classe;
- geração automática de insights básicos;
- botão para testar integração com o backend em `/api/v1/fia/simular`;
- build estático compatível com GitHub Pages no repositório `ADCRS`.

### Rodar localmente somente o frontend
...
### Rodar frontend + backend com Docker
...
O workflow `.github/workflows/pages.yml` compila o frontend e publica `frontend/dist` no GitHub Pages.

No GitHub, confirme:

1. `Settings > Pages > Source = GitHub Actions`;
2. branch principal = `main`;
3. workflow `Deploy Frontend to GitHub Pages` executado com sucesso.

> Observação: no GitHub Pages o frontend roda em modo estático. O cálculo MACA funciona localmente no navegador. A integração com o backend só funcionará se a API FastAPI estiver publicada em algum servidor e `VITE_API_URL` apontar para ela.
